...
const ProductsSortBar = ({
  ...
  order,
  sortBy,
}: { className?: string } & ISortSearchParams) => {
  ...
  return (
    <div className={twMerge("flex gap-1 items-center", className)}>
      ...
      <Button
        className={
          `flex-1 min-w-fit` +
          ` ${sortBy === "pop" ? "m-and-t:bg-slate-300 m-and-t:text-black" : ""}`
        }
        aria-label="Phổ biến"
      ...
      <Button
        className={
          `flex-1 min-w-fit` +
          ` ${sortBy === "ctime" ? "m-and-t:bg-slate-300 m-and-t:text-black" : ""}`
        }
        aria-label="Mới nhất"
      ...
      <Button
        className={
          `flex-1 min-w-fit` +
          ` ${sortBy === "sales" ? "m-and-t:bg-slate-300 m-and-t:text-black" : ""}`
        }
        aria-label="Bán chạy"
      ...
      <div className="relative self-stretch bg-white items-center rounded-sm flex justify-between min-w-fit m-and-t:border-r-[1px] m-and-t:border-black/20">
        <div
          ...
          <span className="capitalize">
            {order === "asc" ? "Giá thấp đến cao" : order === "desc" ? "Giá cao đến thấp" : "Giá"}
          </span>
          <IoIosArrowDown />
        </div>
        <div
          ...
          {order && (
            <FaArrowDownLong
              className={clsx(
                "fill-primary transition-all duration-150 ease-linear",
  ...
  return (
    <div
      className={clsx(
        ...
        isScrollDown &&
          "m-and-t:-translate-y-[calc(var(--header-mobile-height)_+_var(--products-mobile-sort-bar))]",
