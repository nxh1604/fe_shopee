...
const ModalOverlay = ({ children, isOpen, handleClose }: ModalOverlayProps) => {
  return (
    <div
      onClick={handleClose}
      className={`${!isOpen && "hidden"} fixed bottom-0 top-0 left-0 right-0 bg-black/30 flex items-center justify-center z-50`}
    >
      {children}
    </div>
  );
...
const ModalContent = ({ children, handleClose }: ModalCotentProps) => {
  return (
    <div className="relative animate-[ModalAnimate_0.2s] will-change-[opacity] " onClick={(e) => e.stopPropagation()}>
