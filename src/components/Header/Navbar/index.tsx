...
const DropdownContent = ({
  ...
  return (
    <div className={"hidden hover:block absolute peer-hover:block " + ` ${className}`}>
      {children}
    </div>
  );
};
  ...
  return (
    <nav className="*:flex *:gap-3 *:px-2 text-nowrap py-2 flex justify-between text-sm ">
      ...
      <ul className="*:flex *:items-center justify-end">
        ...
        <DropdownWrapper>
          <DropdownHover>
            <li className="hover:opacity-80 flex items-center gap-1">
              <FaGlobe className="w-4 h-4" /> Tiếng Việt <FaChevronDown className="w-4 h-4" />
            </li>
          </DropdownHover>
          <DropdownContent className="right-0 top-[90%] z-10 pt-3">
            <ul className="origin-[calc(100%-20px)_top] text-sm text-black shadow  relative before:triangle-up animate-[headerNotify_0.3s] w-[180px] will-change-[opacity,_scale] bg-white">
