      ...
      <div className="bg-white h-full">
        ...
        <ul className="space-y-2 pl-2 pb-5 text-sm">
          {categories.map((category) => {
            return (
              <li onClick={() => setIsActive(category.id)} key={category.id} className={twMerge("flex items-center group relative left-1")}>
                <IoMdArrowDropright
                  className={twMerge(
                ...
                />
                <span className={`group-hover:text-primary ${isActive === category.id && "text-primary"}`}>{category.name}</span>
              </li>
            );
          })}
        </ul>
