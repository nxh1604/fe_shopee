...
const RatingStar = ({
  ...
  return (
    <div className="relative w-max ml-auto">
      ...
      >
        <IoStarOutline className={clsx(`w-[${starSize}px] h-[${starSize}px] ${fillColor === "primary" ? "text-primary" : "text-yellow-500"}`)} />
        <IoStarOutline className={clsx(`w-[${starSize}px] h-[${starSize}px] ${fillColor === "primary" ? "text-primary" : "text-yellow-500"}`)} />
        <IoStarOutline className={clsx(`w-[${starSize}px] h-[${starSize}px] ${fillColor === "primary" ? "text-primary" : "text-yellow-500"}`)} />
        <IoStarOutline className={clsx(`w-[${starSize}px] h-[${starSize}px] ${fillColor === "primary" ? "text-primary" : "text-yellow-500"}`)} />
        <IoStarOutline className={clsx(`w-[${starSize}px] h-[${starSize}px] ${fillColor === "primary" ? "text-primary" : "text-yellow-500"}`)} />
      ...
      <span
        ...
        // }}
        style={{ width: `${width}%` }}
        className={clsx("overflow-hidden absolute translate-y-[-50%] left-0 top-[50%]", `h-[${starSize}px]`)}
      >
        <IoStarSharp
          style={{ left: `${gaps[0]}px` }}
          className={clsx("absolute", `w-[${starSize}px] h-[${starSize}px] ${fillColor === "primary" ? "fill-primary" : "  fill-yellow-500"}`)}
        />
        <IoStarSharp
