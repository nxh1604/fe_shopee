...
const DropdownWrapper = ({ children }: { children: React.ReactNode }) => {
  return <div className="hover:opacity-80 group relative">{children}</div>;
};

const DropdownHover = ({ children }: { children: React.ReactNode }) => {
  return <div>{children}</div>;
};

const DropdownContent = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="hidden hover:block absolute z-10 top-[90%] group-hover:block w-[200px] space-y-2 pt-3">
      {children}
    </div>
  );
};
...
const Navbar = (): JSX.Element => {
  return (
    <header className="*:max-w-[1200px] *:w-full *:min-w-fit *:mx-auto shopee-linear-gradient-b  pb-5 min-w-fit text-white">
      <div className="*:flex *:gap-3 *:px-2 text-nowrap py-2 flex justify-between text-sm ">
        <ul className="justify-start ">
          {links.map((link) => {
            if (link.title === "Tải ứng dụng")
              return (
                <li key={link.title} className="group relative">
                  <Link
                    className="border-l-2 pl-3 border-slate-300 hover:opacity-80"
                    href={link.link}
                  >
                    {link.title}
                  </Link>
                  <div className="hidden hover:opacity-100 animate-[fadeIn_0.2s] hover:block absolute z-10 top-[90%] group-hover:block w-[200px] space-y-2 pt-3">
            ...
            return (
              <li
                className="hover:opacity-80 first:border-none first:pl-0 border-l-2 pl-3 border-slate-300"
                key={link.title}
              >
          ...
          <li className="flex items-cente gap-2 border-l-2 pl-3 border-slate-300">
            Kết nối{" "}
            <Link className="hover:opacity-60 self-center" href={"#"}>
        ...
        <ul className="*:flex *:items-center justify-end">
          <li className="group gap-1 relative">
            <span className="hover:opacity-80 cursor-pointer">
              <FaRegBell className="inline-block w-4 h-4" /> Thông báo
            </span>
            {user && (
              <div className="hidden origin-[calc(100%-20px)_top] animate-[headerNotify_0.3s] group-hover:block hover:block absolute top-full w-[404px] right-0 pt-3 text-wrap text-justify">
                <div className="shadow-xl bg-white relative text-black before:triangle-up">
                      ...
                      <Link href="#" className="flex  items-start gap-3">
                        ...
                        <div className="flex-1 gap-1">
                          <p>
                            ...
                            quo id, repudiandae vitae ex. Adipisci, quam. Odio, quod cumque!
                          </p>
            ...
            {!user && (
              <div className="hidden origin-[calc(100%-20px)_top] animate-[headerNotify_0.3s] group-hover:flex hover:flex absolute top-full w-[404px] right-0 pt-3 h-[400px]">
      ...
      <div className="flex justify-between pt-4 pb-2 px-2 items-end gap-5">
        ...
        <div className="flex-1 flex gap-1 text-black bg-white p-1">
          <input type="text" className="w-full p-2 text-base" />
...
const ShopeeLogo = ({ className }: { className: string }): JSX.Element => {
  return (
    <svg viewBox="0 0 192 65" className={className}>
      <g fillRule="evenodd">
